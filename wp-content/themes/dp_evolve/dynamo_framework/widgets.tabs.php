...
class DP_Tabs_Widget extends WP_Widget {
	...
	function DP_Tabs_Widget() {
		parent::__construct(
			...
			array( 
				...
				'description' => __( 'Use this widget to show tabs created form the selected sidebar', 'dp-theme') 
			)
		);
		...
		if ($sidebar !== '') {
			...
			foreach($widget_code as $code) {
				...
				if(count($title_match) > 1 && isset($title_match[1][0])) {
					...
					array_push($tabs, _e('No title specified!', 'dp-theme'));
				}
	...
	?>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'sidebar' ) ); ?>"><?php _e( 'Tabs source:', 'dp-theme' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('event')); ?>"><?php _e('Tabs activator event:', 'dp-theme'); ?></label>
		...
		<p>
			...
			<select id="<?php echo esc_attr( $this->get_field_id('autoanim')); ?>" name="<?php echo esc_attr( $this->get_field_name('autoanim')); ?>">
				<option value="enabled"<?php echo (esc_attr($autoanim) == 'enabled') ? ' selected="selected"' : ''; ?>>
					<?php _e('Enabled', 'dp-theme'); ?>
				</option>
				<option value="disabled"<?php echo (esc_attr($autoanim) == 'disabled') ? ' selected="selected"' : ''; ?>>
					<?php _e('Disabled', 'dp-theme'); ?>
				</option>
			</select>
		</p>
		
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'anim_speed' ) ); ?>"><?php _e( 'Animation speed (ms):', 'dp-theme' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'anim_interval' ) ); ?>"><?php _e( 'Animation interval (ms):', 'dp-theme' ); ?></label>
