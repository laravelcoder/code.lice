...
/**
 * 
 * DP News Show Pro Widget class
 *
 **/


class DP_NSP_Widget extends WP_Widget {
         ...
         *
         **/
        function DP_NSP_Widget() {
                parent::__construct(
                        'widget_dp_nsp', 
                        __('DP News Show Pro', 'dp-theme'), 
                        array( 
                                'classname' => 'widget_dp_nsp', 
                                'description' => __( 'Use this widget to show recent items with images and additional elements like title, date etc.', 'dp-theme') 
                        ),
                        array(
                                'width' => 670, 
                                'height' => 350
                ...
                $this->alt_option_name = 'widget_dp_nsp';
                //
                ...
                //
                add_action('wp_enqueue_scripts', array('DP_NSP_Widget', 'add_scripts'));
        }
        
        static function add_scripts() {
                wp_register_script( 'dp-nsp', dynamo_file_uri('js/widgets/nsp.js'), array('jquery'), false, true);
                wp_enqueue_script('dp-nsp');
        }
        ...
        function widget($args, $instance) {
                ...
                $cache_uri = get_template_directory_uri() . '/dynamo_framework/cache_nsp/';
                ...
                // wrap articles
                echo '<div class="dp-nsp-arts">';
                echo '<div class="dp-nsp-arts-scroll dp-nsp-pages-'.$amount_of_art_pages.'">';
                //
                $i = 0;
                //
                for($p = 0; $p < $amount_of_art_pages; $p++) {
                        echo '<div class="dp-nsp-arts-page dp-nsp-cols-'.$amount_of_art_pages.'">';
                                ...
                                if(isset($results[$i]) || (is_array($results[0]) && isset($results[0][$i]))) {
                                        ...
                                        for($j = 1; $j <= 5; $j++) {
                                                ...
                                                if(
                                                        ...
                                                        $art_output .= '<div class="dp-nsp-content-wrap">';
                                                }
                                        ...
                                        echo '<div class="dp-nsp-art dp-nsp-cols-'.$config['article_cols'].'" '.$style.'>' . $art_output . '</div>';
                                }
                                //
                                $i++;
                        }
                ...
                if($amount_of_art_pages > 1) {
                        echo '<div class="dp-nsp-arts-nav">';
                        
                        if($config['article_pagination'] != 'arrows') {
                                echo '<ul class="dp-nsp-pagination">';
                        ...
                        if($config['article_pagination'] != 'pagination') {
                                echo '<div class="dp-nsp-prev">&laquo;</div>';
                                echo '<div class="dp-nsp-next">&raquo;</div>';
                        }
                ...
                if($amount_of_articles <= count($results)) {
                        ...
                        if($amount_of_links > 0) {
                                echo '<div class="dp-nsp-links">';
                                echo '<div class="dp-nsp-links-scroll dp-nsp-pages-'.ceil($amount_of_links / $this->wdgt_config['links_rows']).'">';
                                
                                for($i = 0; $i < ceil($amount_of_links / $this->wdgt_config['links_rows']); $i++) {
                                        echo '<ul class="dp-nsp-list dp-nsp-cols-'.$config['links_pages'].'">';
                ...
                if($this->wdgt_config['links_rows'] > 0 && ceil($amount_of_links / $this->wdgt_config['links_rows']) > 1) {
                        echo '<div class="dp-nsp-links-nav">';
        ...
        function update( $new_instance, $old_instance ) {
                ...
                if(isset($alloptions['widget_dp_nsp'])) {
                        delete_option( 'widget_dp_nsp' );
                }
        ...
        function refresh_cache() {
                if(is_array(get_option('widget_widget_dp_nsp'))) {
                    $ids = array_keys(get_option('widget_widget_dp_nsp'));
                    for($i = 0; $i < count($ids); $i++) {
                        if(is_numeric($ids[$i])) {
                            delete_transient(md5('widget_dp_nsp-' . $ids[$i]));
                        }
                    }
            } else {
                    delete_transient(md5('widget_dp_nsp-' . $this->id));
            }
        }
        ...
        ?>        
                <div class="dp-nsp-col">
                        <h3><?php _e('Basic settings', 'dp-theme'); ?></h3>
                        ...
                        <h3><?php _e('Data source settings', 'dp-theme'); ?></h3>
                        
                        <p>
                                <label for="<?php echo esc_attr( $this->get_field_id( 'data_source_type' ) ); ?>"><?php _e( 'Data source:', 'dp-theme' ); ?></label>
                        ...
                        <p>
                                <label for="<?php echo esc_attr( $this->get_field_id( 'orderby' ) ); ?>"><?php _e( 'Order by:', 'dp-theme' ); ?></label>
                                ...
                                <select id="<?php echo esc_attr( $this->get_field_id('order')); ?>" name="<?php echo esc_attr( $this->get_field_name('order')); ?>">
                                        ...
                                        <option value="DESC"<?php selected($order, 'DESC'); ?>>
                                                <?php _e('DESC', 'dp-theme'); ?>
                                        </option>
                                </select>
                        </p>
                        
                        <p>
                                <label for="<?php echo esc_attr( $this->get_field_id( 'offset' ) ); ?>"><?php _e( 'Offset:', 'dp-theme' ); ?></label>
                        ...
                        <?php if(is_multisite()) : ?>
                        <p>
                                <label for="<?php echo esc_attr( $this->get_field_id( 'data_source_blog' ) ); ?>"><?php _e( 'Blog ID (leave blank for current blog): ', 'dp-theme' ); ?></label>
                        ...
                        <p>
                                <h3><?php _e('Articles amount', 'dp-theme'); ?></h3>
                                <label for="<?php echo esc_attr( $this->get_field_id( 'article_pages' ) ); ?>"><?php _e( 'pages:', 'dp-theme' ); ?></label>
                                ...
                                <label for="<?php echo esc_attr( $this->get_field_id( 'article_cols' ) ); ?>"><?php _e( 'columns:', 'dp-theme' ); ?></label>
                                ...
                                <label for="<?php echo esc_attr( $this->get_field_id( 'article_rows' ) ); ?>"><?php _e( 'rows:', 'dp-theme' ); ?></label>
                        ...
                        <p>
                                <h3><?php _e('Links amount', 'dp-theme'); ?></h3>
                                <label for="<?php echo esc_attr( $this->get_field_id( 'links_pages' ) ); ?>"><?php _e( 'pages:', 'dp-theme' ); ?></label>
                                ...
                                <label for="<?php echo esc_attr( $this->get_field_id( 'links_rows' ) ); ?>"><?php _e( 'rows:', 'dp-theme' ); ?></label>
                        ...
                        <p>
                                <label for="<?php echo esc_attr( $this->get_field_id( 'article_pagination' ) ); ?>"><?php _e( 'Article pagination:', 'dp-theme' ); ?></label>
                                ...
                                <select id="<?php echo esc_attr( $this->get_field_id('article_pagination')); ?>" name="<?php echo esc_attr( $this->get_field_name('article_pagination')); ?>">
                                        <option value="pagination"<?php selected($article_pagination, 'pagination'); ?>>
                                                <?php _e('Pagination', 'dp-theme'); ?>
                                        </option>
                                        <option value="arrows"<?php selected($article_pagination, 'arrows'); ?>>
                                                <?php _e('Arrows', 'dp-theme'); ?>
                                        </option>
                                        <option value="pagination_with_arrows"<?php selected($article_pagination, 'pagination_with_arrows'); ?>>
                                                <?php _e('Both', 'dp-theme'); ?>
                                        </option>
                                </select>
                        </p>
                        
                        <p>
                                <label for="<?php echo esc_attr( $this->get_field_id( 'links_pagination' ) ); ?>"><?php _e( 'Links pagination:', 'dp-theme' ); ?></label>
                        ...
                        <h3><?php _e('Autoanimation settings', 'dp-theme'); ?></h3>
                        
                        <p>
                                <label for="<?php echo esc_attr( $this->get_field_id( 'autoanim' ) ); ?>"><?php _e( 'Auto-animation:', 'dp-theme' ); ?></label>
                        ...
                        <p>
                                <label for="<?php echo esc_attr( $this->get_field_id( 'autoanim_interval' ) ); ?>"><?php _e( 'Interval (ms):', 'dp-theme' ); ?></label>
                        ...
                        <p>
                                <label for="<?php echo esc_attr( $this->get_field_id( 'autoanim_hover' ) ); ?>"><?php _e( 'Auto-animation stops on hover:', 'dp-theme' ); ?></label>
                ...
                <div class="dp-nsp-col">
                        <h3><?php _e('Article format', 'dp-theme'); ?></h3>        
                        
                        <p>                                
                                <label for="<?php echo esc_attr( $this->get_field_id( 'article_block_padding' ) ); ?>"><?php _e( 'Padding:', 'dp-theme' ); ?></label>
                        ...
                        <p>
                                <label for="<?php echo esc_attr( $this->get_field_id( 'article_title_state' ) ); ?>"><?php _e('Title', 'dp-theme'); ?></label>
                                ...
                                <label for="<?php echo esc_attr( $this->get_field_id( 'article_title_len' ) ); ?>"><?php _e( 'length:', 'dp-theme' ); ?></label>
                                ...
                                <select id="<?php echo esc_attr( $this->get_field_id('article_title_len_type')); ?>" name="<?php echo esc_attr( $this->get_field_name('article_title_len_type')); ?>">
                                        <option value="chars"<?php selected($article_title_len_type, 'chars'); ?>>
                                                <?php _e('Chars', 'dp-theme'); ?>
                                        </option>
                                        <option value="words"<?php selected($article_title_len_type, 'words'); ?>>
                                                <?php _e('Words', 'dp-theme'); ?>
                                        </option>
                                </select>
                                
                                <span class="dp-right">
                                        <label for="<?php echo esc_attr( $this->get_field_id( 'article_title_order' ) ); ?>"><?php _e( 'order:', 'dp-theme' ); ?></label>
                                        
                                        <select id="<?php echo esc_attr( $this->get_field_id('article_title_order')); ?>" name="<?php echo esc_attr( $this->get_field_name('article_title_order')); ?>" class="dp-order">
                        ...
                        <p>
                                <label for="<?php echo esc_attr( $this->get_field_id( 'article_text_state' ) ); ?>"><?php _e('Text', 'dp-theme'); ?></label>
                                ...
                                <label for="<?php echo esc_attr( $this->get_field_id( 'article_text_len' ) ); ?>"><?php _e( 'length:', 'dp-theme' ); ?></label>
                                ...
                                <span class="dp-right">
                                        <label for="<?php echo esc_attr( $this->get_field_id( 'article_text_order' ) ); ?>"><?php _e( 'order:', 'dp-theme' ); ?></label>
                                        
                                        <select id="<?php echo esc_attr( $this->get_field_id('article_text_order')); ?>" name="<?php echo esc_attr( $this->get_field_name('article_text_order')); ?>" class="dp-order">
                        ...
                        <p>
                                <label for="<?php echo esc_attr( $this->get_field_id( 'article_image_state' ) ); ?>"><?php _e('Image', 'dp-theme'); ?></label>
                                ...
                                <label for="<?php echo esc_attr( $this->get_field_id( 'article_image_w' ) ); ?>"><?php _e( 'size:', 'dp-theme' ); ?></label>
                                ...
                                <span class="dp-right">
                                        <label for="<?php echo esc_attr( $this->get_field_id( 'article_image_order' ) ); ?>"><?php _e( 'order:', 'dp-theme' ); ?></label>
                                        
                                        <select id="<?php echo esc_attr( $this->get_field_id('article_image_order')); ?>" name="<?php echo esc_attr( $this->get_field_name('article_image_order')); ?>" class="dp-order">
                        ...
                        <p class="dp-indent">                                
                                <label for="<?php echo esc_attr( $this->get_field_id( 'image_block_padding' ) ); ?>"><?php _e( 'Margin:', 'dp-theme' ); ?></label>
                        ...
                        <p class="dp-indent">
                                <label for="<?php echo esc_attr( $this->get_field_id( 'article_image_pos' ) ); ?>"><?php _e( 'Position:', 'dp-theme' ); ?></label>
                                ...
                                <select id="<?php echo esc_attr( $this->get_field_id('article_image_pos')); ?>" name="<?php echo esc_attr( $this->get_field_name('article_image_pos')); ?>">
                                        <option value="top"<?php selected($article_image_pos, 'top'); ?>><?php _e('Top', 'dp-theme'); ?></option>
                                        <option value="left"<?php selected($article_image_pos, 'left'); ?>><?php _e('Left', 'dp-theme'); ?></option>
                                </select>
                        </p>        
                                                
                        <p>
                                <label for="<?php echo esc_attr( $this->get_field_id( 'article_info_state' ) ); ?>"><?php _e('Info block', 'dp-theme'); ?></label>
                                ...
                                <span class="dp-right">
                                        <label for="<?php echo esc_attr( $this->get_field_id( 'article_info_order' ) ); ?>"><?php _e( 'order:', 'dp-theme' ); ?></label>
                                        
                                        <select id="<?php echo esc_attr( $this->get_field_id('article_info_order')); ?>" name="<?php echo esc_attr( $this->get_field_name('article_info_order')); ?>" class="dp-order">
                        ...
                        <p class="dp-indent">
                                <label for="<?php echo esc_attr( $this->get_field_id( 'article_info_format' ) ); ?>"><?php _e( 'Format:', 'dp-theme' ); ?></label>
                        ...
                        <p class="dp-indent">        
                                <label for="<?php echo esc_attr( $this->get_field_id( 'article_info_date_format' ) ); ?>"><?php _e( 'Date format:', 'dp-theme' ); ?></label>
                        ...
                        <p>
                                <label for="<?php echo esc_attr( $this->get_field_id( 'article_readmore_state' ) ); ?>"><?php _e('Read more', 'dp-theme'); ?></label>
                                ...
                                <span class="dp-right">
                                        <label for="<?php echo esc_attr( $this->get_field_id( 'article_readmore_order' ) ); ?>"><?php _e( 'order:', 'dp-theme' ); ?></label>
                                        
                                        <select id="<?php echo esc_attr( $this->get_field_id('article_readmore_order')); ?>" name="<?php echo esc_attr( $this->get_field_name('article_readmore_order')); ?>" class="dp-order">
                        ...
                        <h3><?php _e('Link format', 'dp-theme'); ?></h3>
                                                
                        <p>
                                <label for="<?php echo esc_attr( $this->get_field_id( 'link_title_state' ) ); ?>"><?php _e('Title', 'dp-theme'); ?></label>
                                ...
                                <label for="<?php echo esc_attr( $this->get_field_id( 'links_title_len' ) ); ?>"><?php _e( 'length:', 'dp-theme' ); ?></label>
                        ...
                        <p>
                                <label for="<?php echo esc_attr( $this->get_field_id( 'link_text_state' ) ); ?>"><?php _e('Text', 'dp-theme'); ?></label>
                                ...
                                <label for="<?php echo esc_attr( $this->get_field_id( 'links_text_len' ) ); ?>"><?php _e( 'length:', 'dp-theme' ); ?></label>
                        ...
                        <h3><?php _e('Cache settings', 'dp-theme'); ?></h3>
                        
                        <p>
                                <label for="<?php echo esc_attr( $this->get_field_id( 'cache_time' ) ); ?>"><?php _e( 'Cache time (min):', 'dp-theme' ); ?></label>
         ...
         **/
         ...
         function generate_art_title($i) {
                 ...
                 $output = '<h3 class="dp-nsp-header"><a href="'.$art_url.'" title="'.str_replace('"','', $art_title).'">'.$art_title_short.'</a></h3>';
                 
                 return apply_filters('dp_nsp_art_title', $output);
         }
         ...
         function generate_art_text($i) {
                 ...
                 $output = '<p class="dp-nsp-text">'.$art_text.'</p>';
                 
                 return apply_filters('dp_nsp_art_text', $output);
         }
                         ...
                         if(!is_wp_error($img_editor)) {
                                 ...
                                 if(is_string($new_path)) {
                                         ...
                                         if($this->wdgt_config['article_image_pos'] == 'left' && $this->wdgt_config['article_image_order'] == 1) {
                                                 return apply_filters('dp_nsp_art_image', '<div class="dp-nsp-image-wrap"><a href="'.$art_url.'" class="dp-image-link"><img src="'.$new_path.'" alt="" class="dp-nsp-image" '.$style.' height="'.$this->wdgt_config['article_image_h'].'" width="'.$this->wdgt_config['article_image_w'].'" /></a></div>');
                                         } else {
                                                 return apply_filters('dp_nsp_art_image', '<a href="'.$art_url.'" class="dp-responsive dp-image-link"><img src="'.$new_path.'" alt="" class="dp-nsp-image dp-responsive" '.$style.' height="'.$this->wdgt_config['article_image_h'].'" width="'.$this->wdgt_config['article_image_w'].'" /></a>');
                                         }
                                 } else {
                                         return _e('An error1 occured during creating the thumbnail.', 'dp-theme');
                                 }
                         } else {
                                 return _e('An error2 occured during creating the thumbnail.', 'dp-theme');
         ...
         function generate_art_info($i) {
                 ...
                 if(stripos($this->wdgt_config['article_info_format'], '%CATEGORY') !== FALSE) {
                         ...
                         if(count($categories) > 0) {
                                 foreach($categories as $cat) {                         
                                         $category .= ' <a href="'.get_category_link( $cat->term_id ).'" class="dp-nsp-category">'.$cat->name.'</a> ';
                                 }
                         }
                 }
                 ...
                 if(stripos($this->wdgt_config['article_info_format'], '%AUTHOR') !== FALSE) {                         
                         ...
                         $author = '<a href="'.get_author_posts_url($author_ID).'" class="dp-nsp-author">'.$username.'</a>';
                 }
                 ...
                 if(stripos($this->wdgt_config['article_info_format'], '%DATE') !== FALSE) {
                         ...
                         $date = '<span class="dp-nsp-date">' . get_the_time($this->wdgt_config['article_info_date_format'], $art_ID) . '</span>';
                 }
                 ...
                 if(stripos($this->wdgt_config['article_info_format'], '%COMMENTS') !== FALSE) {
                         ...
                         if($comment_count == 0) {
                                 $comment_phrase = _e('No comments', 'dp-theme');
                         } else if($comment_count >= 1) {
                                 $comment_phrase = _e('Comments ', 'dp-theme') . '(' . $comment_count . ')';
                         }
                 ...
                 return apply_filters('dp_nsp_art_info', '<p class="dp-nsp-info">' . $output . '</p>');
         }
         ...
         function generate_art_readmore($i) {
                 ...
                 $output = '<a href="'.$art_url.'" class="readon btn" title="'._e('Read more', 'dp-theme').'">'._e('Read more', 'dp-theme').'</a>';
                 
                 return apply_filters('dp_nsp_art_readmore', $output);
         }
          ...
          **/
          ...
          function generate_link_title($i) {
                  ...
                  $output = '<h4 class="dp-nsp-link-header"><a href="'.$art_url.'" title="'.str_replace('"','', $art_title).'">'.$art_title_short.'</a></h4>';
                  
                  return apply_filters('dp_nsp_link_title', $output);
          }
          ...
          function generate_link_text($i) {
                  ...
                  $output = '<p class="dp-nsp-link-text">'.$art_text.'</p>';
                  
                  return apply_filters('dp_nsp_link_text', $output);
          }
