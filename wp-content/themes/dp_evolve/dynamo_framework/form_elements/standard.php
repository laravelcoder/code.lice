...
class DPFormInputTextBlock extends DPFormInput {
	public function output() {
		...
		$output = '<p class="dpTextBlock" data-visible="true"'.($this->class).' '.($this->visibility).'">'.($this->label).'</p>';
	...
	public function output() {
		$output = '<p data-visible="true"><label 
				...
				>'.$this->label.'</label>
					...
					data-name="'.($this->name).'"
				>';
		$output .= '<option value="N"'.selected($this->value, 'N', false).'>'.__('Disabled', 'dp-theme').'</option>';
		$output .= '<option value="Y"'.selected($this->value, 'Y', false).'>'.__('Enabled', 'dp-theme').'</option>';
	...
	public function output() {
		...
		$output = '<p data-visible="true"><label 
			...
			<input 
				...
				data-name="'.($this->name).'"
			/>
			<input id="'.($this->tpl->name).'_'.($this->name).'_button" class="dpMedia" type="button" value="'.__('Upload Image', 'dp-theme').'" />
			<input id="'.($this->tpl->name).'_'.($this->name).'_button1" class="dpMedia" type="button" value="'.__('Remove Image', 'dp-theme').'" />
			<small>'.__('Enter an URL or upload an image.', 'dp-theme').'</small>
...
class DPFormInputBackground extends DPFormInput {
	public function output() {
		...
		/>
		<br/>
		<span id="'.($this->tpl->name).'_'.($this->name).'_bg" class="dpPattern" style="background: url('.get_bloginfo("template_url").'/images/'.$this->other->folder.'/'.$this->value.'.png)"></span>
		</p>';
	...
	public function output() {
		$output = '<p data-visible="true"><label 
				...
				<select 
					...
					data-name="'.($this->name).'"
				>';
				$output .= '<option value="none"'.selected($this->value, 'not', false).'>'.__('--Select--', 'dp-theme').'</option>';
				$slug = 'slideshows';
