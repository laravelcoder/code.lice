...
jQuery(window).on('beforeunload', function () {
    jQuery.cookie('dp_last_opened_widget_rules_wrap', 0, {
        expires: 365,
        path: '/'
    });
});
...
jQuery(document).click(function (e) {
    if (jQuery(e.target).hasClass('dp_widget_rules_btn')) {
        ...
        if (wrap.hasClass('active')) {
            ...
            jQuery.cookie('dp_last_opened_widget_rules_wrap', wrap.attr('data-id'), {
                expires: 365,
                path: '/'
            });
        }
...
function dp_widget_control_init(id, inner) {
    // check if the widget isn't a new widget
    var allForms = jQuery('.dp_widget_rules_form');
    var newest = null;
    var flag = 0;
    ...
    if (!inner) {
        ...
        if (flag > 1) {
            ...
            dp_widget_control_init('#' + newest.attr('id'), true);
            return;
        }
    }
    ...
    if (inner) {
        var mouseUpEvent = function () {
            setTimeout(function () {
                dp_widget_control_init_events(id, inner);
...
function dp_widget_control_init_events(id, inner) {
    var form = jQuery(id);
    ...
    if (form.attr('data-state') !== 'initialized') {
        ...
        var firstSelect = form.parent().find('.dp_widget_rules_select');
        var select = form.children('.dp_widget_rules_form_select');
        var btn = form.find('.dp_widget_rules_btn');
        var form_inputs = {};
        ...
        var selectStyles = jQuery(document).find('.dp_widget_rules_select_styles');
        selectStyles.each(function (i, select) {
            ...
            if (!select.hasClass('initialized')) {
                select.change(function () {
                    ...
                    if (value !== 'dpcustom') {
                        if (!field.hasClass('dp-unvisible')) {
                            field.addClass('dp-unvisible');
                        }
                    } else {
                        if (field.hasClass('dp-unvisible')) {
                            field.removeClass('dp-unvisible');
                        }
                    }
                });
