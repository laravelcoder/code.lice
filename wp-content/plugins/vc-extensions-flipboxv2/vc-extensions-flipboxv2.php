<?php
/*
Plugin Name: Visual Composer Add-on - Flip Box V2
Description: Help you add Flip Box in the Visual Composer, with more options in the back box.
Author: Sike
Version: 1.0
...
function vc_extensions_flipboxv2_notice(){
    $plugin_data = get_plugin_data(__FILE__);
    echo '
    <div class="updated">
      <p>'.sprintf(__('<strong>%s</strong> requires <strong><a href="http://codecanyon.net/item/visual-composer-page-builder-for-wordpress/242431?ref=sike" target="_blank">Visual Composer</a></strong> plugin to be installed and activated on your site.', 'vc_extensions_flipboxv2'), $plugin_data['Name']).'</p>
    </div>';
  }
  if (!defined('ABSPATH')) die('-1');


  function vc_extensions_flipboxv2_init(){
    if (!defined('WPB_VC_VERSION')) {add_action('admin_notices', 'vc_extensions_flipboxv2_notice'); return;}
    wp_register_style( 'vc_extensions_admin_flipboxv2', plugins_url('css/admin_icon.css', __FILE__) );
    wp_enqueue_style( 'vc_extensions_admin_flipboxv2' );
    if(class_exists('VC_Extensions_flipboxv2')&&!function_exists('cq_vc_flipboxv2_func')) $vc_extensions_flipboxv2 = new VC_Extensions_flipboxv2();
  }

  add_action('init', 'vc_extensions_flipboxv2_init');
