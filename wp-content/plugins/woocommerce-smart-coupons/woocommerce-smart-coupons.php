		...
		class WC_Smart_Coupons {
			...
			public function sc_endpoint_content() {
				...
				<div id='sc_coupons_list'>
					<h5><?php echo __( 'Store Credits', self::$text_domain ); ?></h5>
					...
					<?php
						if ( !empty( $total_store_credit ) && $total_store_credit != 0 ) {
							?>
							<div class="wc_sc_total_available_store_credit"><?php echo sprintf( __( 'Total Credit Amount: %s', self::$text_domain ), wc_price( $total_store_credit ) ); ?></div>
							<?php
						}
					?>
				<br><hr />
			...
			public function show_available_coupons( $available_coupons_heading = '', $page = 'checkout' ) {
				...
				?>
				<div id="coupons_list" style="display: none;"><h3><?php _e( stripslashes( $available_coupons_heading ), self::$text_domain ) ?></h3><div id="all_coupon_container">
					<?php
			...
			public function modify_paypal_args( $args, $order ) {
				...
				if ( $discount_amount_cart > $item_total ) {
					...
					foreach ( $coupons as $item_id => $item ) {
						...
						$code = trim( $item['name'] );
						$coupon = new WC_Coupon( $code );
						if ( $coupon->discount_type == 'smart_coupon' && ! empty( $item['discount_amount'] ) ) {
							$new_discount = 0;
							...
							if ( $discount_updated ) {
								...
								$smart_coupons_contribution = get_post_meta( $order->id, 'smart_coupons_contribution', true );
								...
								$order_note[] = sprintf(__( '%s worth of Store Credit restored to coupon %s.', self::$text_domain ), '<strong>' . wc_price( $cut_amount ) . '</strong>', '<code>' . $code . '</code>' );
							}
						}
					}
					$order->set_total( $order_total, 'total' );
					if ( ! empty( $order_note ) ) {
						$note = sprintf(__( '%s Because PayPal doesn\'t accept discount on shipping & tax.', self::$text_domain ), implode( ', ', $order_note ) );
