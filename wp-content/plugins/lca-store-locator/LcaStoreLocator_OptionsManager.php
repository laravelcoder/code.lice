...
class LcaStoreLocator_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'lca-store-locator'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'lca-store-locator'); ?></h2>
            <table cellspacing="1" cellpadding="2"><tbody>
            <tr><td><?php _e('System', 'lca-store-locator'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'lca-store-locator'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'lca-store-locator');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'lca-store-locator'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'lca-store-locator');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'lca-store-locator'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'lca-store-locator') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'lca-store-locator');
            case 'false':
                return __('false', 'lca-store-locator');

            case 'Administrator':
                return __('Administrator', 'lca-store-locator');
            case 'Editor':
                return __('Editor', 'lca-store-locator');
            case 'Author':
                return __('Author', 'lca-store-locator');
            case 'Contributor':
                return __('Contributor', 'lca-store-locator');
            case 'Subscriber':
                return __('Subscriber', 'lca-store-locator');
            case 'Anyone':
                return __('Anyone', 'lca-store-locator');
        }
        return $optionValue;
    }
