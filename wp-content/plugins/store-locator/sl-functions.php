...
//$sl_google_map_domain=sl_data('sl_google_map_domain');
    if (empty($sl_vars)) {
        ...
        $sl_vars['height'] = sl_data('sl_map_height');
        $sl_vars['width'] = sl_data('sl_map_width');
        $sl_vars['width_units'] = sl_data('sl_map_width_units');
        $sl_vars['height_units'] = sl_data('sl_map_height_units');
        $sl_vars['radii'] = sl_data('sl_map_radii');
        $sl_vars['icon'] = sl_data('sl_map_home_icon');
        $sl_vars['icon2'] = sl_data('sl_map_end_icon2');
        $sl_vars['google_map_domain'] = sl_data('sl_google_map_domain');
        $sl_vars['google_map_country'] = sl_data('sl_google_map_country');
        $sl_vars['theme'] = sl_data('sl_map_theme');
        $sl_vars['location_table_view'] = sl_data('sl_location_table_view');
        $sl_vars['search_label'] = sl_data('sl_search_label');
        $sl_vars['zoom_level'] = sl_data('sl_zoom_level');
        $sl_vars['use_city_search'] = sl_data('sl_use_city_search');
        $sl_vars['use_name_search'] = sl_data('sl_use_name_search');
        $sl_vars['name'] = sl_data('sl_name');
        $sl_vars['radius_label'] = sl_data('sl_radius_label');
        $sl_vars['website_label'] = sl_data('sl_website_label');
        $sl_vars['directions_label'] = sl_data('sl_directions_label');
        $sl_vars['num_initial_displayed'] = sl_data('sl_num_initial_displayed');
        $sl_vars['load_locations_default'] = sl_data('sl_load_locations_default');
        $sl_vars['distance_unit'] = sl_data('sl_distance_unit');
        $sl_vars['map_overview_control'] = sl_data('sl_map_overview_control');
        $sl_vars['admin_locations_per_page'] = sl_data('sl_admin_locations_per_page');
        $sl_vars['instruction_message'] = sl_data('sl_instruction_message');
        $sl_vars['map_character_encoding'] = sl_data('sl_map_character_encoding');
        $sl_vars['start'] = sl_data('sl_start');
        $sl_vars['map_language'] = sl_data('sl_map_language');
        $sl_vars['map_region'] = sl_data('sl_map_region');
        $sl_vars['sensor'] = sl_data('sl_sensor');
        $sl_vars['geolocate'] = sl_data('sl_geolocate');
        $sl_vars['map_type'] = sl_data('sl_map_type');
        $sl_vars['remove_credits'] = sl_data('sl_remove_credits');
        $sl_vars['api_key'] = sl_data('store_locator_api_key');
        $sl_vars['load_results_with_locations_default'] = sl_data('sl_load_results_with_locations_default');
        $sl_vars['city_dropdown_label'] = sl_data('sl_city_dropdown_label');
    }

    if (strlen(trim($sl_vars['geolocate'])) == 0) {
        $sl_vars['geolocate'] = "0";
    }
    $sl_geolocate = $sl_vars['geolocate'];

    if (strlen(trim($sl_vars['sensor'])) == 0) {
        $sl_vars['sensor'] = ($sl_vars['geolocate'] == '1') ? "true" : "false";
    }
    $sl_sensor = $sl_vars['sensor'];

    if ($sl_vars['map_region'] === null) {
        $sl_vars['map_region'] = "";
    }
    $sl_map_region = $sl_vars['map_region'];

    if (strlen(trim($sl_vars['map_language'])) == 0) {
        $sl_vars['map_language'] = "en";
    }
    $sl_map_language = $sl_vars['map_language'];

    if (strlen(trim($sl_vars['start'])) == 0) {
        $sl_vars['start'] = date("Y-m-d H:i:s");
    }
    $sl_start = $sl_vars['start'];

    if ($sl_vars['map_character_encoding'] === null) {
        $sl_vars['map_character_encoding'] = "";
    }
    $sl_map_character_encoding = $sl_vars['map_character_encoding'];

    if ($sl_vars['instruction_message'] === null) {
        $sl_vars['instruction_message'] = "Enter Your Address or Zip Code Above.";
    }
    $sl_instruction_message = $sl_vars['instruction_message'];

    if (strlen(trim($sl_vars['city_dropdown_label'])) == 0) {
        $sl_vars['city_dropdown_label'] = "--" . __("Search By City",
            SL_TEXT_DOMAIN) . "--";
    }
    $sl_city_dropdown_label = $sl_vars['city_dropdown_label'];

    if (empty($sl_vars['location_not_found_message']) || $sl_vars['location_not_found_message'] === null) {
        $sl_vars['location_not_found_message'] = "";
    }
    $sl_location_not_found_message = $sl_vars['location_not_found_message'];

    if (empty($sl_vars['no_results_found_message']) || $sl_vars['no_results_found_message'] === null) {
        $sl_vars['no_results_found_message'] = "No Results Found";
    }
    $sl_no_results_found_message = $sl_vars['no_results_found_message'];

    if (strlen(trim($sl_vars['admin_locations_per_page'])) == 0) {
        $sl_vars['admin_locations_per_page'] = "100";
    }
    $sl_admin_locations_per_page = $sl_vars['admin_locations_per_page'];

    if (strlen(trim($sl_vars['map_overview_control'])) == 0) {
        $sl_vars['map_overview_control'] = "0";
    }
    $sl_map_overview_control = $sl_vars['map_overview_control'];

    if (strlen(trim($sl_vars['distance_unit'])) == 0) {
        $sl_vars['distance_unit'] = "miles";
    }
    $sl_distance_unit = $sl_vars['distance_unit'];

    if (strlen(trim($sl_vars['load_locations_default'])) == 0) {
        $sl_vars['load_locations_default'] = "1";
    }
    $sl_load_locations_default = $sl_vars['load_locations_default'];

    if (strlen(trim($sl_vars['load_results_with_locations_default'])) == 0) {
        $sl_vars['load_results_with_locations_default'] = "1";
    }
    $sl_load_results_with_locations_default = $sl_vars['load_results_with_locations_default'];

    if (strlen(trim($sl_vars['num_initial_displayed'])) == 0) {
        $sl_vars['num_initial_displayed'] = "500";
    }
    $sl_num_initial_displayed = $sl_vars['num_initial_displayed'];

    if (strlen(trim($sl_vars['website_label'])) == 0) {
        $sl_vars['website_label'] = "Website";
    }
    $sl_website_label = $sl_vars['website_label'];

    if (strlen(trim($sl_vars['directions_label'])) == 0) {
        $sl_vars['directions_label'] = "Directions";
    }
    $sl_directions_label = $sl_vars['directions_label'];

    if (strlen(trim($sl_vars['radius_label'])) == 0) {
        $sl_vars['radius_label'] = "Radius";
    }
    $sl_radius_label = $sl_vars['radius_label'];

    if (strlen(trim($sl_vars['map_type'])) == 0) {
        $sl_vars['map_type'] = "google.maps.MapTypeId.ROADMAP";
    } elseif ($sl_vars['map_type'] == "G_NORMAL_MAP") {
        $sl_vars['map_type'] = 'google.maps.MapTypeId.ROADMAP';
    } elseif ($sl_vars['map_type'] == "G_SATELLITE_MAP") {
        $sl_vars['map_type'] = 'google.maps.MapTypeId.SATELLITE';
    } elseif ($sl_vars['map_type'] == "G_HYBRID_MAP") {
        $sl_vars['map_type'] = 'google.maps.MapTypeId.HYBRID';
    } elseif ($sl_vars['map_type'] == "G_PHYSICAL_MAP") {
        $sl_vars['map_type'] = 'google.maps.MapTypeId.TERRAIN';
    }
    $sl_map_type = $sl_vars['map_type'];

    if (strlen(trim($sl_vars['remove_credits'])) == 0) {
        $sl_vars['remove_credits'] = "0";
    }
    $sl_remove_credits = $sl_vars['remove_credits'];

    if (strlen(trim($sl_vars['use_name_search'])) == 0) {
        $sl_vars['use_name_search'] = "0";
    }
    $sl_use_name_search = $sl_vars['use_name_search'];

    if (strlen(trim($sl_vars['use_city_search'])) == 0) {
        $sl_vars['use_city_search'] = "1";
    }
    $sl_use_city_search = $sl_vars['use_city_search'];

    if (strlen(trim($sl_vars['name'])) == 0) {
        $sl_vars['name'] = "LotsOfLocales";
    }
    $sl_name = $sl_vars['name'];

    if (strlen(trim($sl_vars['zoom_level'])) == 0) {
        $sl_vars['zoom_level'] = "4";
    }
    $sl_zoom_level = $sl_vars['zoom_level'];

    if (strlen(trim($sl_vars['search_label'])) == 0) {
        $sl_vars['search_label'] = "Address";
    }
    $sl_search_label = $sl_vars['search_label'];

    if (strlen(trim($sl_vars['location_table_view'])) == 0) {
        $sl_vars['location_table_view'] = "Normal";
    }
    $sl_location_table_view = $sl_vars['location_table_view'];

    if ($sl_vars['theme'] === null) {
        $sl_vars['theme'] = "";
    }
    $sl_theme = $sl_vars['theme'];

    if (strlen(trim($sl_vars['google_map_country'])) == 0) {
        $sl_vars['google_map_country'] = "United States";
    }
    $sl_google_map_country = $sl_vars['google_map_country'];

    if (strlen(trim($sl_vars['google_map_domain'])) == 0) {
        $sl_vars['google_map_domain'] = "maps.google.com";
    }
    $sl_google_map_domain = $sl_vars['google_map_domain'];

    if (strlen(trim($sl_vars['icon2'])) == 0) {
        $sl_vars['icon2'] = SL_ICONS_BASE . "/droplet_red.png";
    }
    $sl_icon2 = $sl_vars['icon2'];

    if (strlen(trim($sl_vars['icon'])) == 0) {
        $sl_vars['icon'] = SL_ICONS_BASE . "/droplet_green.png";
    }
    $sl_icon = $sl_vars['icon'];

    if (strlen(trim($sl_vars['height'])) == 0) {
        $sl_vars['height'] = "350";
    }
    $sl_height = $sl_vars['height'];

    if (strlen(trim($sl_vars['height_units'])) == 0) {
        $sl_vars['height_units'] = "px";
    }
    $sl_height_units = $sl_vars['height_units'];

    if (strlen(trim($sl_vars['width'])) == 0) {
        $sl_vars['width'] = "100";
    }
    $sl_width = $sl_vars['width'];

    if (strlen(trim($sl_vars['width_units'])) == 0) {
        $sl_vars['width_units'] = "%";
    }
    $sl_width_units = $sl_vars['width_units'];

    if (strlen(trim($sl_vars['radii'])) == 0) {
        $sl_vars['radii'] = "1,5,10,25,(50),100,200,500";
    }
    $sl_radii = $sl_vars['radii'];

    if ($sl_vars['api_key'] === null) {
        $sl_vars['api_key'] = "";
    }
    $sl_api_key = $sl_vars['api_key'];

    if (empty($sl_vars['scripts_load']) || strlen(trim($sl_vars['scripts_load'])) == 0) {
        $sl_vars['scripts_load'] = "selective";
    }
...
function sl_install_tables()
{
    ...
    $sql = "CREATE TABLE " . $table_name . " (
			...
			sl_type varchar(255) NULL,
			sl_facebook_phone_number varchar(255) NULL,
			sl_website varchar(255) NULL,
			sl_seo_landing_page varchar(255) NULL,
			sl_real_phone varchar(255) NULL,
			sl_ppc_landing_page varchar(255) NULL,
			sl_landing_page varchar(255) NULL,
			sl_first_name varchar(255) NULL,
			sl_last_name varchar(255) NULL,
			sl_category varchar(255) NULL,
			sl_action varchar(255) NULL,
			sl_label varchar(255) NULL,
			sl_youtube_phone_number varchar(255) NULL,
			sl_category_facebook varchar(255) NULL,
			sl_category_youtube varchar(255) NULL,
			sl_lcoa_locator varchar(255) NULL,
			PRIMARY KEY  (sl_id)
    ...
    if ($wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s",
        $table_name)) != $table_name || $wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s",
        $table_name_2)) != $table_name_2 || $wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s",
...
function sl_head_scripts()
{
    ...
    if ($sl_vars['scripts_load'] != 'all') {
        ...
        $on_sl_page = $wpdb->get_results("SELECT post_name, post_content FROM " . SL_DB_PREFIX . "posts WHERE LOWER(post_content) LIKE '%[store-locator%' AND (post_name='$pagename' OR ID='" . esc_sql($_GET['p']) . "' OR ID='" . esc_sql($_GET['page_id']) . "')",
          ARRAY_A);
        //Checking if code used in posts
        $sl_code_is_used_in_posts = $wpdb->get_results("SELECT post_name, ID FROM " . SL_DB_PREFIX . "posts WHERE LOWER(post_content) LIKE '%[store-locator%' AND post_type='post'",
          ARRAY_A);
    ...
    if ($sl_vars['scripts_load'] == 'all' || $on_sl_page || is_search() || ((is_archive() || is_404()) && $sl_code_is_used_in_posts) || is_front_page() || is_single($post_ids_array) || !is_singular(array(
        'page',
        'attachment',
        'post'
      )) || function_exists('show_sl_scripts') || is_page_template()
    ) {
...
//        print "<script src='https://maps.googleapis.com/maps/api/js?v=3{$sens}{$lang_loc}{$region_loc}{$key}{$lib}' type='text/javascript'></script>\n";
//        print "<script src='//ajax.googleapis.com/ajax/libs/jquery/1.8.3/jquery.min.js'></script>\n";
        ...
        $sl_page_ids = $wpdb->get_results("SELECT ID FROM " . SL_DB_PREFIX . "posts WHERE LOWER(post_content) LIKE '%[store-locator%' AND post_status='publish'",
          ARRAY_A);
...
function sl_add_options_page()
{
    ...
    $sl_menu_pages['main'] = array(
      'title' => __("Store Locator", SL_TEXT_DOMAIN),
      'capability' => 'administrator',
      'page_url' => $parent_url,
      'icon' => SL_BASE . '/images/logo.ico.png',
      'menu_position' => 47
    );
    $sl_menu_pages['sub']['information'] = array(
      'parent_url' => $parent_url,
      'title' => __("News & Upgrades", SL_TEXT_DOMAIN),
      'capability' => 'administrator',
      'page_url' => $parent_url
    );
    $sl_menu_pages['sub']['locations'] = array(
      'parent_url' => $parent_url,
      'title' => __("Locations", SL_TEXT_DOMAIN),
      'capability' => 'administrator',
      'page_url' => SL_PAGES_DIR . '/locations.php'
    );
    $sl_menu_pages['sub']['mapdesigner'] = array(
      'parent_url' => $parent_url,
      'title' => __("MapDesigner", SL_TEXT_DOMAIN),
      'capability' => 'administrator',
      'page_url' => SL_PAGES_DIR . '/mapdesigner.php'
    );
...
{
    if (function_exists('do_sl_hook')) {
        do_sl_hook('sl_menu_pages_filter', '', array(&$sl_menu_pages));
    }

    foreach ($sl_menu_pages as $menu_type => $value) {
        if ($menu_type == 'main') {
            add_menu_page($value['title'], $value['title'],
              $value['capability'], $value['page_url'], '', $value['icon'],
              $value['menu_position']);
        }
        if ($menu_type == 'sub') {
            foreach ($value as $sub_value) {
                add_submenu_page($sub_value['parent_url'], $sub_value['title'],
                  $sub_value['title'], $sub_value['capability'],
                  $sub_value['page_url']);
            }
        }
    }
...
{
    if (function_exists("do_sl_hook")) {
        do_sl_hook("sl_where_clause_filter");
    }
}
...
function sl_add_admin_javascript()
{
    ...
    wp_enqueue_script('prettyPhoto', SL_JS_BASE . "/jquery.prettyPhoto.js",
      "jQuery");
    ...
    $admin_js = "
        ...
        var sl_addons_base=sl_uploads_base+'" . str_replace(SL_UPLOADS_BASE, '',
        SL_ADDONS_BASE) . "';
        var sl_addons_path=sl_uploads_path+'" . str_replace(SL_UPLOADS_PATH, '',
        SL_ADDONS_PATH) . "';
        var sl_includes_base=sl_base+'" . str_replace(SL_BASE, '',
        SL_INCLUDES_BASE) . "';
        var sl_includes_path=sl_path+'" . str_replace(SL_PATH, '',
        SL_INCLUDES_PATH) . "';
        var sl_cache_path=sl_uploads_path+'" . str_replace(SL_UPLOADS_PATH, '',
        SL_CACHE_PATH) . "';
        var sl_pages_base=sl_base+'" . str_replace(SL_BASE, '',
        SL_PAGES_BASE) . "'";
    print preg_replace("@[\\\]@", "\\\\\\",
      $admin_js); //Windows-based server path backslash escape fix
    print "</script>\n";
    if (preg_match("@add-locations\.php|locations\.php@",
      $_SERVER['REQUEST_URI'])) {
...
function sl_set_query_defaults()
{
    ...
    $where = (!empty($_GET['q'])) ? $wpdb->prepare(" WHERE sl_store LIKE '%%%s%%' OR sl_address LIKE '%%%s%%' OR sl_city LIKE '%%%s%%' OR sl_state LIKE '%%%s%%' OR sl_zip LIKE '%%%s%%' OR sl_tags LIKE '%%%s%%'",
        $_GET['q'], $_GET['q'], $_GET['q'], $_GET['q'], $_GET['q'],
        $_GET['q']) . " " . $extra : ""; //die($where);
...
function do_hyperlink(&$text, $target = "'_blank'", $type = "both")
{
    if ($type == "both" || $type == "protocol") {
        ...
        $text = preg_replace("@[a-zA-Z]+://([.]?[a-zA-Z0-9_/?&amp;%20,=\-\+\-\#])+@s",
          "<a href=\"\\0\" target=$target>\\0</a>", $text);
    }
    if ($type == "both" || $type == "noprotocol") {
        // match www.something
        $text = preg_replace("@(^| )(www([.]?[a-zA-Z0-9_/=-\+-\#])*)@s",
          "\\1<a href=\"http://\\2\" target=$target>\\2</a>", $text);
    }
    return $text;
}
...
if (!function_exists('addon_activation_message')) {
    function addon_activation_message($url_of_upgrade = "")
    {
        ...
        print "<div style='background-color:#eee; border:solid silver 1px; padding:7px; color:black; display:block;'>" . __("You haven't activated this upgrade yet",
            SL_TEXT_DOMAIN) . ". ";
        if (function_exists('do_sl_hook') && !preg_match("/addons\-platform/",
            $url_of_upgrade)
        ) {
            print "<a href='" . SL_ADDONS_PAGE . "'>" . __("Activate",
                SL_TEXT_DOMAIN) . "</a></div><br>";
        } else {
            print __("Go to pull-out Dashboard, and activate under 'Activation Keys' section.",
                SL_TEXT_DOMAIN) . "</div><br>";
        }
    }
}
/*-----------------------------------------------------------*/
...
function sl_truncate($var, $length = 50, $mode = "return", $type = 1)
{
    ...
    if (strlen($var) > $length) {
        if ($type == 1) { //avoids cutting words in half
            ...
            $var = preg_replace("@[[:space:]]{1}.{1,10}$@s", "",
              $var); //making sure it doesn't cut word in half
            $var = $var . "...";
...
function sl_process_tags($tag_string, $db_action = "insert", $sl_id = "")
{
    ...
    if (!is_array($sl_id) && preg_match("@,@", $sl_id)) {
        ...
        $rplc_arr = array_fill(0, count($sl_id),
          "%d"); //var_dump($rplc_arr); //die();
        $id_string = implode(",",
          array_map(array($wpdb, "prepare"), $rplc_arr, $sl_id));
...
function sl_ty($file)
{
    ...
    $ty['text'] = urlencode(__("Love it! I've made my site more user-friendly with",
        SL_TEXT_DOMAIN) . " LotsOfLocales #WP #WordPress #StoreLocator #GoogleMaps");
    $ty['text2'] = urlencode(__("Great! I can now easily display my locations using",
        SL_TEXT_DOMAIN) . " LotsOfLocales #GoogleMaps #StoreLocator #WordPress #WP");
    $ty['is_included'] = (basename($file) != basename($_SERVER['SCRIPT_FILENAME'])) ? true : false;
    if (!$ty['is_included']) {
        $ty['thanks_msg'] = __("<b>Like the Store Locator? Help guide others' decisions:</b><br><a href='#' class='star_button'>Give my review now</a> -- 1 sentence is plenty<br><br><b>Any problems? View:</b><br><a href='http://docs.viadat.com/' target='_blank'>Documentation</a> / <a href='http://www.viadat.com/contact/' target='_blank'>Contact us</a>",
            SL_TEXT_DOMAIN) . "<br><br>";
        $ty['thanks_msg_style'] = "style='line-height:20px; font-familty:helvetica; text-align:left; font-size:15px'";
        $ty['thanks_heading'] = "<br>" . __("My Views",
            SL_TEXT_DOMAIN) . "<br><br>";
        ...
        $ty['thanks_msg'] = __("<b>Liking this? Help guide others' decisions:</b>&nbsp;<a href='#' class='star_button'>Give my review now</a><br><b>Any problems? View:</b>&nbsp;<a href='http://docs.viadat.com/' target='_blank'>Documentation</a> / <a href='http://www.viadat.com/contact/' target='_blank'>Contact us</a>",
            SL_TEXT_DOMAIN) . "<br><br>";
        $ty['thanks_msg_style'] = "";
        $ty['thanks_heading'] = "";
        $ty['action_call'] = "";
    ...
    $ty['done_msg'] = __("Done!  I've tweeted about / rated it",
      SL_TEXT_DOMAIN);
...
function sl_prepare_tag_string($sl_tags)
{
    ...
    if (substr($sl_tags, strlen($sl_tags) - 1,
        1) != "," && trim($sl_tags) != ""
    ) {
        $sl_tags .= ",";
    }
...
function sl_data($setting_name, $i_u_d_s = "select", $setting_value = "")
{
    ...
    if ($i_u_d_s == "insert" || $i_u_d_s == "add" || $i_u_d_s == "update") {
        ...
        $exists = $wpdb->get_var($wpdb->prepare("SELECT sl_setting_id FROM " . SL_SETTING_TABLE . " WHERE sl_setting_name = %s",
          $setting_name));
        if (!$exists) {
            $q = $wpdb->prepare("INSERT INTO " . SL_SETTING_TABLE . " (sl_setting_name, sl_setting_value) VALUES (%s, %s)",
              $setting_name, $setting_value);
        } else {
            $q = $wpdb->prepare("UPDATE " . SL_SETTING_TABLE . " SET sl_setting_value = %s WHERE sl_setting_name = %s",
              $setting_value, $setting_name);
        }
        $wpdb->query($q);
    } elseif ($i_u_d_s == "delete") {
        $q = $wpdb->prepare("DELETE FROM " . SL_SETTING_TABLE . " WHERE sl_setting_name = %s",
          $setting_name);
        $wpdb->query($q);
    } elseif ($i_u_d_s == "select" || $i_u_d_s == "get") {
        $q = $wpdb->prepare("SELECT sl_setting_value FROM " . SL_SETTING_TABLE . " WHERE sl_setting_name = %s",
          $setting_name);
        $r = $wpdb->get_var($q);
...
var sl_location_not_found_message='$lnf';\n";
    ...
    $home_size = (function_exists("getimagesize") && file_exists($home_icon_path)) ? getimagesize($home_icon_path) : array(
      0 => 26,
      1 => 35
    );
    ...
    $end_size = (function_exists("getimagesize") && file_exists($end_icon_path)) ? getimagesize($end_icon_path) : array(
      0 => 26,
      1 => 35
    );
...
function sl_location_form($mode = "add", $pre_html = "", $post_html = "")
{
	...
	<tr>
		...
		<b>" . __("The General Address Format",
        SL_TEXT_DOMAIN) . ": </b>(<a href=\"#\" onclick=\"show('format'); return false;\">" . __("show/hide",
        SL_TEXT_DOMAIN) . "</a>)
		<span id='format' style='display:none'><br><i>" . __("Name of Location",
        SL_TEXT_DOMAIN) . "<br>
		...
		" . __("Address",
        SL_TEXT_DOMAIN) . "<br><input name='sl_address' size=21>&nbsp;<small>(" . __("Street - Line1",
        SL_TEXT_DOMAIN) . ")</small><br>
		<input name='sl_address2' size=21>&nbsp;<small>(" . __("Street - Line 2 - optional",
        SL_TEXT_DOMAIN) . ")</small><br>
		<table cellpadding='0px' cellspacing='0px'><tr><td style='padding-left:0px' class='nobottom'><input name='sl_city' size='21'><br><small>" . __("City",
        SL_TEXT_DOMAIN) . "</small></td>
		<td><input name='sl_state' size='7'><br><small>" . __("State",
        SL_TEXT_DOMAIN) . "</small></td>
		<td><input name='sl_zip' size='10'><br><small>" . __("Zip",
        SL_TEXT_DOMAIN) . "</small></td></tr></table><br>
		</div><div style='display:inline; width:50%'>
		" . __("Additional Information", SL_TEXT_DOMAIN) . "<br>
		<textarea name='sl_description' rows='5' cols='17'></textarea>&nbsp;<small>" . __("Description",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_tags'>&nbsp;<small>" . __("Tags (seperate with commas)",
        SL_TEXT_DOMAIN) . "</small><br>
		...
		<input name='sl_image'>&nbsp;<small>" . __("Image URL (shown with location)",
        SL_TEXT_DOMAIN) . "</small>
		<input name='sl_type'>&nbsp;<small>" . __("Type", SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_facebook_phone_number'>&nbsp;<small>" . __("Facebook Phone Number",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_website'>&nbsp;<small>" . __("Website",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_seo_landing_page'>&nbsp;<small>" . __("SEO Landing Page",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_real_phone'>&nbsp;<small>" . __("Real Phone",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_ppc_landing_page'>&nbsp;<small>" . __("PPC Landing Page",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_landing_page'>&nbsp;<small>" . __("Landing Page",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_first_name'>&nbsp;<small>" . __("First Name",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_last_name'>&nbsp;<small>" . __("Last Name",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_category'>&nbsp;<small>" . __("Category",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_action'>&nbsp;<small>" . __("Action", SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_label'>&nbsp;<small>" . __("Label", SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_youtube_phone_number'>&nbsp;<small>" . __("YouTube Phone Number",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_category_facebook'>&nbsp;<small>" . __("Category Facebook",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_category_youtube'>&nbsp;<small>" . __("Category YouTube",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_lcoa_locator'>&nbsp;<small>" . __("LCOA Locator",
        SL_TEXT_DOMAIN) . "</small><br>
		";

    $html .= (function_exists("do_sl_hook")) ? do_sl_hook("sl_add_location_fields",
      "append-return") : "";
    ...
    foreach ($_POST as $key => $value) {
        if (preg_match("@sl_@", $key)) {
            ...
            if (is_array($value)) {
                ...
                $valueList .= $wpdb->prepare("%s",
                    comma(stripslashes($value))) . ",";
...
function sl_single_location_info($value, $colspan, $bgcol)
{
	...
	<tr>
		<td style='vertical-align:top !important; width:30%'><b>" . __("Name of Location",
        SL_TEXT_DOMAIN) . "</b><br><input name='sl_store-$value[sl_id]' id='sl_store-$value[sl_id]' value='$value[sl_store]' size=30><br><br>
		<b>" . __("Address",
        SL_TEXT_DOMAIN) . "</b><br><input name='sl_address-$value[sl_id]' id='sl_address-$value[sl_id]' value='$value[sl_address]' size='13'>&nbsp;<small>(" . __("Street - Line1",
        SL_TEXT_DOMAIN) . ")</small><br>
		<input name='sl_address2-$value[sl_id]' id='sl_address2-$value[sl_id]' value='$value[sl_address2]' size='13'>&nbsp;<small>(" . __("Street - Line 2 - optional",
        SL_TEXT_DOMAIN) . ")</small><br>
		<table cellpadding='0px' cellspacing='0px'><tr><td style='padding-left:0px' class='nobottom'><input name='sl_city-$value[sl_id]' id='sl_city-$value[sl_id]' value='$value[sl_city]' size='13'><br><small>" . __("City",
        SL_TEXT_DOMAIN) . "</small></td>
		<td><input name='sl_state-$value[sl_id]' id='sl_state-$value[sl_id]' value='$value[sl_state]' size='4'><br><small>" . __("State",
        SL_TEXT_DOMAIN) . "</small></td>
		<td><input name='sl_zip-$value[sl_id]' id='sl_zip-$value[sl_id]' value='$value[sl_zip]' size='6'><br><small>" . __("Zip",
        SL_TEXT_DOMAIN) . "</small></td></tr></table>";

    if (function_exists("do_sl_hook")) {
        sl_show_custom_fields();
    }

    $cancel_onclick = "location.href=\"" . str_replace("&edit=$_GET[edit]", "",
        $_SERVER['REQUEST_URI']) . "\"";
    print "<br><br>
		<nobr><input type='submit' value='" . __("Update",
        SL_TEXT_DOMAIN) . "' class='button-primary'><input type='button' class='button' value='" . __("Cancel",
        SL_TEXT_DOMAIN) . "' onclick='$cancel_onclick'></nobr>
		</td><td style='width:30%; vertical-align:top !important;'>
		<b>" . __("Additional Information", SL_TEXT_DOMAIN) . "</b><br>
		<textarea name='sl_description-$value[sl_id]' id='sl_description-$value[sl_id]' rows='5' cols='17'>$value[sl_description]</textarea>&nbsp;<small>" . __("Description",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_tags-$value[sl_id]' id='sl_tags-$value[sl_id]' value='$value[sl_tags]' size='13'>&nbsp;<small>" . __("Tags (seperate with commas)",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_url-$value[sl_id]' id='sl_url-$value[sl_id]' value='$value[sl_url]' size='13'>&nbsp;<small>" . __("URL",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_hours-$value[sl_id]' id='sl_hours-$value[sl_id]' value='$value[sl_hours]' size='13'>&nbsp;<small>" . __("Hours",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_phone-$value[sl_id]' id='sl_phone-$value[sl_id]' value='$value[sl_phone]' size='13'>&nbsp;<small>" . __("Phone",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_fax-$value[sl_id]' id='sl_fax-$value[sl_id]' value='$value[sl_fax]' size='13'>&nbsp;<small>" . __("Fax",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_email-$value[sl_id]' id='sl_email-$value[sl_id]' value='$value[sl_email]' size='13'>&nbsp;<small>" . __("Email",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_image-$value[sl_id]' id='sl_image-$value[sl_id]' value='$value[sl_image]' size='13'>&nbsp;<small>" . __("Image URL (shown with location)",
        SL_TEXT_DOMAIN) . "</small>
		<input name='sl_type-$value[sl_id]' id='sl_fax-$value[sl_id]' value='$value[sl_type]' size='13'>&nbsp;<small>" . __("Type",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_facebook_phone_number-$value[sl_id]' id='sl_fax-$value[sl_id]' value='$value[sl_facebook_phone_number]' size='13'>&nbsp;<small>" . __("Facebook Phone Number",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_website-$value[sl_id]' id='sl_fax-$value[sl_id]' value='$value[sl_website]' size='13'>&nbsp;<small>" . __("Facebook Website",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_seo_landing_page-$value[sl_id]' id='sl_fax-$value[sl_id]' value='$value[sl_seo_landing_page]' size='13'>&nbsp;<small>" . __("SEO Landing Page",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_real_phone-$value[sl_id]' id='sl_fax-$value[sl_id]' value='$value[sl_real_phone]' size='13'>&nbsp;<small>" . __("Real Phone",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_ppc_landing_page-$value[sl_id]' id='sl_fax-$value[sl_id]' value='$value[sl_ppc_landing_page]' size='13'>&nbsp;<small>" . __("PPC Landing Page",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_landing_page-$value[sl_id]' id='sl_fax-$value[sl_id]' value='$value[sl_landing_page]' size='13'>&nbsp;<small>" . __("Landing Page",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_first_name-$value[sl_id]' id='sl_fax-$value[sl_id]' value='$value[sl_first_name]' size='13'>&nbsp;<small>" . __("First Name",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_last_name-$value[sl_id]' id='sl_fax-$value[sl_id]' value='$value[sl_last_name]' size='13'>&nbsp;<small>" . __("Last Name",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_category-$value[sl_id]' id='sl_fax-$value[sl_id]' value='$value[sl_category]' size='13'>&nbsp;<small>" . __("Category",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_action-$value[sl_id]' id='sl_fax-$value[sl_id]' value='$value[sl_action]' size='13'>&nbsp;<small>" . __("Action",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_label-$value[sl_id]' id='sl_fax-$value[sl_id]' value='$value[sl_label]' size='13'>&nbsp;<small>" . __("Label",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_youtube_phone_number-$value[sl_id]' id='sl_fax-$value[sl_id]' value='$value[sl_youtube_phone_number]' size='13'>&nbsp;<small>" . __("YouTube Phone Number",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_category_facebook-$value[sl_id]' id='sl_fax-$value[sl_id]' value='$value[sl_category_facebook]' size='13'>&nbsp;<small>" . __("Category Facebook",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_category_youtube-$value[sl_id]' id='sl_fax-$value[sl_id]' value='$value[sl_category_youtube]' size='13'>&nbsp;<small>" . __("Category YouTube",
        SL_TEXT_DOMAIN) . "</small><br>
		<input name='sl_lcoa_locator-$value[sl_id]' id='sl_fax-$value[sl_id]' value='$value[sl_lcoa_locator]' size='13'>&nbsp;<small>" . __("LCOA Locator",
        SL_TEXT_DOMAIN) . "</small><br>";

    print "</td><td style='vertical-align:top !important; width:40%'>";
    if (function_exists("do_sl_hook")) {
        do_sl_hook("sl_single_location_edit", "select-top");
    }
    print "</td></tr>
	</table>
</form></td>";
...
//TOC pt.1
    ...
    preg_match_all("@\=\=[ ]([^\=\=]+)[ ]\=\=@", $toc,
      $toc_match_1); //var_dump($toc_match_1); die();
    preg_match_all("@\=[ ]([^\=]+)[ ]\=@", $toc,
      $toc_match_2); //var_dump($toc_match_2); die();
    $toc_cont = "";
    foreach ($toc_match_2[1] as $heading) {
        if (!in_array($heading, $toc_match_1[1]) && !in_array($heading,
            $toc_match_0[1]) && !preg_match("@^[0-9]+\.[0-9]+@", $heading)
        ) {
...
//$txt=str_replace("== ", "<div id='wphead' style='color:black; background: -moz-linear-gradient(center bottom, #D7D7D7, #E4E4E4) repeat scroll 0 0 transparent'><h1 id='site-heading'><span id='site-title'>", $txt);
    ...
    $txt = str_replace(" ==",
      "</th></thead></table><!--a style='float:right' href='#readme_toc'>Table of Contents</a-->",
      $txt);
    $txt = str_replace("= ", "$h3_start", $txt);
    $txt = str_replace(" =",
      "</h3><a style='float:right; position:relative; top:-1.5em; font-size:10px' href='#readme_toc'>" . __("table of contents",
        SL_TEXT_DOMAIN) . "</a>", $txt);
    $txt = preg_replace("@Tags:[ ]?[^\r\n]+\r\n@", "", $txt);

//TOC pt. 2
    $txt = str_replace("</h2>",
      "</h2><a name='readme_toc'></a><div style='float:right;  width:500px; border-radius:1em; border:solid silver 1px; padding:7px; padding-top:0px; margin:10px; margin-right:0px;'><h3>" . __("Table of Contents",
        SL_TEXT_DOMAIN) . "</h2>$toc_cont</div>", $txt);
    $txt = preg_replace_callback("@$h2_start<u>([^<.]*)</u></h1>@s",
      create_function('$matches',
        'return "<h2 style=\'font-family:Georgia; margin-bottom:0.05em;\'><a name=\'".comma($matches[1])."\'></a>$matches[1]</u></h1>";'),
      $txt);
    $txt = preg_replace_callback("@$th_start([^<.]*)</th>@s",
      create_function('$matches',
        'return "<th style=\'font-size:125%; font-weight:bold;\'><a name=\'".comma($matches[1])."\'></a>$matches[1]</th>";'),
      $txt);
    $txt = preg_replace_callback("@$h3_start( )*([^<.]*)( )*</h3>@s",
      create_function('$matches',
        'return "<h3 style=\'font-family:Georgia; margin-bottom:0.05em; margin-top:0.3em\'><a name=\"".comma($matches[2])."\"></a>{$matches[1]}$matches[2]</h3>";'),
      $txt);

//creating hyperlinks on top of labeled URLs (ones preceded w/a label in brackets)
    $txt = preg_replace("@\[([a-zA-Z0-9_/?&amp;\&\ \.%20,=\-\+\-\']+)*\]\(([a-zA-Z]+://)(([.]?[a-zA-Z0-9_/?&amp;%20,=\-\+\-\#]+)*)\)@s",
      "<a onclick=\"window.parent.open('\\2'+'\\3');return false;\" href=\"#\">\\1</a>",
      $txt);
...
//additional formatting
    $txt = preg_replace("@`([^`]+)*`@",
      "<strong class='sl_code code' style='padding:2px; border:0px'>\\1</strong>",
      $txt);
    $txt = preg_replace("@__([^__]+)__@", "<strong>\\1</strong>", $txt);
    $txt = preg_replace("@\r\n([0-9]\.)@", "\r\n&nbsp;&nbsp;&nbsp;\\1", $txt);
    $txt = preg_replace("@([A-Za-z-0-9\/\\&;# ]+): @", "<strong>\\1: </strong>",
      $txt);
...
function sl_admin_toolbar($admin_bar)
{
    ...
    $sl_admin_toolbar_array[] = array(
      'id' => 'sl-menu',
      'title' => __('Store Locator', SL_TEXT_DOMAIN),
      'href' => preg_replace('@wp-admin\/[^\.]+\.php|index\.php@',
        'wp-admin/admin.php', SL_INFORMATION_PAGE),
      'meta' => array(
    ...
    $sl_admin_toolbar_array[] = array(
      ...
      'title' => __('Locations', SL_TEXT_DOMAIN),
      'href' => preg_replace('@wp-admin\/[^\.]+\.php|index\.php@',
        'wp-admin/admin.php', SL_MANAGE_LOCATIONS_PAGE),
      'meta' => array(
        'title' => __('Locations', SL_TEXT_DOMAIN),
        'target' => '_self',
        'class' => 'sl_menu_class'
    ...
    $sl_admin_toolbar_array[] = array(
      ...
      'href' => preg_replace('@wp-admin\/[^\.]+\.php|index\.php@',
        'wp-admin/admin.php', SL_MAPDESIGNER_PAGE),
      'meta' => array(
    ...
    if (function_exists('do_sl_hook')) {
        do_sl_hook('sl_admin_toolbar_filter', '',
          array(&$sl_admin_toolbar_array));
    }
...
function sl_permissions_check()
{
    ...
    if (!empty($_POST['sl_folder_permission'])) {
        @array_map("chmod", $_POST['sl_folder_permission'],
          array_fill(0, count($_POST['sl_folder_permission']), 0755));
    }
    if (!empty($_POST['sl_file_permission'])) {
        //var_dump($_POST['sl_file_permission']); die();
        @array_map("chmod", $_POST['sl_file_permission'],
          array_fill(0, count($_POST['sl_file_permission']), 0644));
    }
    ...
    foreach ($f_to_check as $slf) {
        ...
        $files = new RegexIterator($iterator,
          "/\.(php|gif|jpe?g|png|css|js|csv|xml|json|txt)/");
    ...
    $button_note = __("Note: Clicking this button should update permissions, however, if it doesn\'t, you may need to update permissions by using an FTP program.  Click &quot;OK&quot; or &quot;Confirm&quot; to continue ...",
      SL_TEXT_DOMAIN);

    if ($needs > 0) {
        $output = "";
        print "<br><div class='sl_admin_warning' style='width:97%'><b>" . __("Important Note",
            SL_TEXT_DOMAIN) . ":</b><br>" . __("Some of your folders / files may need updating to the proper permissions (folders: 755 / files: 644), otherwise, all functionality may not work as intended.  View folders / files below",
            SL_TEXT_DOMAIN) . " - <a href='#' onclick='show(\"file_perm_table\"); return false;'>" . __("display / hide list of folders & files",
            SL_TEXT_DOMAIN) . "</a>:<br>
		<div style='float:right'>(<a href='" . $_SERVER['REQUEST_URI'] . "&file_perm_msg=1'>" . __("Hide This Notice Permanently",
            SL_TEXT_DOMAIN) . "</a>)</div><br><br><table cellpadding='7px' id='file_perm_table' style='display:none;'><tr>";
    }
    if (!empty($needs_update["folder"])) {
        $output .= "<td style='vertical-align: top; width:50%'><form method='post' onsubmit=\"return confirm('" . $button_note . "');\"><strong>" . __("Folders",
            SL_TEXT_DOMAIN) . ":</strong><br><input type='submit' class='button-primary' value=\"" . __("Update Checked Folders' Permissions",
            SL_TEXT_DOMAIN) . "\"><br><br>";
        foreach ($needs_update["folder"] as $value) {
            $output .= "\n<input name='sl_folder_permission[]' checked='checked' type='checkbox' value='" . substr($value,
                0, -13) . "'>&nbsp;/" . str_replace(ABSPATH, '',
                $value) . "<br>"; // "-13", removes 13 chars: " - <b> 777 </b>" at end of value
        }
        $output .= "</form></td>";
    }
    if (!empty($needs_update["file"])) {
        $output .= "<td style='vertical-align: top; style: 50%;'><form method='post' onsubmit=\"return confirm('" . $button_note . "');\"><strong>" . __("Files",
            SL_TEXT_DOMAIN) . ":</strong><br><input type='submit' class='button-primary' value=\"" . __("Update Checked Files' Permissions",
            SL_TEXT_DOMAIN) . "\"><br><br>";
        foreach ($needs_update["file"] as $value) {
            $output .= "\n<input name='sl_file_permission[]' checked='checked' type='checkbox' value='" . substr($value,
                0, -13) . "'>&nbsp;/" . str_replace(ABSPATH, '',
                $value) . "<br>";
        }
        $output .= "</form></td>";
...
function sl_remote_data($val_arr)
{
    ...
    if (extension_loaded("curl") && function_exists("curl_init")) {
        ...
        $response = '';
        if (false != ($fs = @fsockopen('viadat.com', 80, $errno, $errstr,
            10))
        ) {
...
if (!function_exists("sl_do_geocoding")) {
    function sl_do_geocoding($address, $sl_id = "")
    {
        if (empty($_POST['no_geocode']) || $_POST['no_geocode'] != 1) {
            ...
            $delay = 100000;
            $ccTLD = $sl_vars['map_region'];
            $sensor = $sl_vars['sensor'];
            $base_url = "https://maps.googleapis.com/maps/api/geocode/json?";

            if ($sensor != "" && !empty($sensor) && ($sensor === "true" || $sensor === "false")) {
                $base_url .= "sensor=" . $sensor;
            } else {
		...
		$base_url .= "&key=".$sl_vars['api_key'];
            ...
            if (strcmp($status, "OK") == 0) {
                ...
                if ($sl_id === "") {
                    $query = sprintf("UPDATE " . SL_TABLE . " SET sl_latitude = '%s', sl_longitude = '%s' WHERE sl_id = '%s' LIMIT 1;",
                      esc_sql($lat), esc_sql($lng),
                      esc_sql($wpdb->insert_id)); //die($query);
                } else {
                    $query = sprintf("UPDATE " . SL_TABLE . " SET sl_latitude = '%s', sl_longitude = '%s' WHERE sl_id = '%s' LIMIT 1;",
                      esc_sql($lat), esc_sql($lng), esc_sql($sl_id));
                }
                ...
                if (strcmp($status, "OVER_QUERY_LIMIT") == 0) {
                    ...
                    // failure to geocode
                    $geocode_pending = false;
                    echo __("Address " . $address . " <font color=red>failed to geocode</font>. ",
                      SL_TEXT_DOMAIN);
                    if (!empty($status)) {
                        echo __("Received status " . $status,
                            SL_TEXT_DOMAIN) . "\n<br>";
                    } else {
                        echo __("No status received from Google",
                            SL_TEXT_DOMAIN) . "\n<br>";
                    }
                }
            }
    ...
    function sl_template($content)
    {
        ...
        if (!preg_match('|\[store-locator|i', $content)) {
            ...
            if ($sl_vars['use_city_search'] == 1) {
                $cs_array = $wpdb->get_results("SELECT CONCAT(TRIM(sl_city), ', ', TRIM(sl_state)) as city_state FROM " . SL_TABLE . " WHERE sl_city<>'' AND sl_state<>'' AND sl_latitude<>'' AND sl_longitude<>'' GROUP BY city_state ORDER BY city_state ASC",
                  ARRAY_A);
...
<div id='sl_div'>
  <form onsubmit='searchLocations(); return false;' id='searchForm' action=''>
    <table border='0' cellpadding='3px' class='sl_header' style='width:$width$width_units;'>
        <tr>
	...
	//$form.="<input name='addressInput3'><input type='hidden' value='1' name='name_search'></td>";
        ...
        <tr>
            ...
            <td valign='top' ";
                ...
                $form .= " ><input $button_style value='Search Locations' id='addressSubmit'/>
            </td>
            <td>
        ...
        $form .= "<table class='map-container' style='cellspacing='0px' cellpadding='0px' >
        <tr>
            <td style='width:100%' valign='top' id='map_td' class='show_div'>

                <div id='sl_map'></div>"

             . "<table cellpadding='0px' class='sl_footer' style='{$hide}' >
                    <tr>
                        <td class='sl_footer_left_column'>
                            <a href='http://www.viadat.com/store-locator' target='_blank' title='WordPress Store Locator -- LotsOfLocales&trade;'>WordPress Store Locator</a></td><td class='sl_footer_right_column'> <a href='http://www.viadat.com' target='_blank' title='Map Maker for Creating Store Locators or Any Address Maps Using WordPress & Google Maps'>Viadat Creations</a>
                        </td>
                    </tr>
                </table>
                <table class='ayy_table'>
                    <tr id='cm_mapTR'>
						<td>
							<p style='text-align:center'><img src='$loading_img' id='loadImg' style='opacity:0; filter:alpha(opacity=0); height:28px; vertical-align:bottom; position:relative;'></p>
							 <div class='text_below_map'>
							 </td>
                           <td width='' valign='top' style='/*display:hidden; border-right:solid silver 1px*/' id='map_sidebar_td'>
                            <div id='map_sidebar' style='height:$height$height_units'>"

                            . " 	$sl_instruction_message</div>
                            </div>
                        </td>
                    </tr>
                </table>
...
'<div class="cws-widget-content doc_search listing_page">
<!--<section class="find_a_doctor">
<form method="get" action="http://liceclinicsofamerica.com/clinics-near-you/" class="doctors-search-form seach_page_form" id="quick-search">
<div class="search_field by_name"><input type="text" placeholder="enter your zip code or city" name="docname"></div>
<div class="search_field by_treatment"><select name="cws-stafftreatments"></option><option value="1">1 mi</option><option value="5">5 mi</option><option value="10">10 mi</option><option value="25">25 mi</option><option value="50">50 mi</option><option value="100" selected="selected">100 mi</option><option value="200">200 mi</option><option value="500">500 mi</option></select></div>
